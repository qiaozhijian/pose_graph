...
//g2o::VertexSim3Expmap; g2o::VertexSE3
...
g2o::VertexSE3Expmap* Optimizer::addSE3ExpNode(const g2o::SE3Quat &pose, const int KFid, const bool& isFixed) {
//g2o::VertexSim3Expmap; g2o::VertexSE3
...
void Optimizer::addSE3ExpEdge(g2o::VertexSE3Expmap* &v1, const Eigen::Isometry3d &iso_pose,
...
void Optimizer::RemoveOutliers(std::vector<g2o::EdgeSE3*>& Myedges){
  //auto e = graph->edges().begin(), edgeEnd = graph->edges().end();
  int num_e = 0;
  for(auto e:Myedges){
    num_e++;
     //e = graph->edges(i);
    ...
    if(e-> chi2() > 5){
      e->setLevel(1);
      LOG(INFO)<< "edge" << num_e << "is an outlier" << endl;
    }
    e->setRobustKernel(0);
  }
  LOG(INFO)<< "Finish removing outliers"<<endl<<"round2"<<endl;
  return ;
}
